...
public class MapRenderer {
    ...
    public MapRenderer(Map map) {
        ...
        this.cam.position.set(map.bob.pos.x, map.bob.pos.y, 10);
        this.cache = new SpriteCache(this.map.tiles.length * this.map.tiles[0].length, false);
        this.blocks =
            new int[(int) Math.ceil(this.map.tiles.length / 24.0f)][(int) Math.ceil(this.map.tiles[0].length / 16.0f)];
    ...
    private void renderBob() {
        ...
        if (map.bob.state == Bob.RUN) {
            if (map.bob.dir == Bob.LEFT) anim = bobLeft;
            else anim = bobRight;
        }
        if (map.bob.state == Bob.IDLE) {
            if (map.bob.dir == Bob.LEFT) anim = bobIdleLeft;
            else anim = bobIdleRight;
        }
        if (map.bob.state == Bob.JUMP) {
            if (map.bob.dir == Bob.LEFT) anim = bobJumpLeft;
            else anim = bobJumpRight;
        }
